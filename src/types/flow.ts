...
import {
    ...
    choiceBuild,
    DynamicChoiceSet,
    dynamicChoiceSetParse,
    dynamicChoiceSetBuild,
    Constant,
    constantParse,
    constantBuild,
    Formula,
            ...
            switch (c1.kind) {
                case sk.Decorator: {
                    ...
                    exp.arguments.forEach((arg) => {
                        ...
                        switch (argIdent) {
                            case 'flow:':
                                f.apiVersion = argValue;
                                v = Number(argValue);
                                break;
                            case 'flow:runInMode':
                                f.runInMode = argValue as FlowRunInMode;
                                break;
                            case 'flow:status':
                                f.status = argValue;
                                break;
                            case 'flow:environments':
                                f.environments = argValue;
                                break;
                            case 'flow:interviewLabel':
                                f.interviewLabel = argValue;
                                break;
                            case 'sourceTemplate:':
                                f.sourceTemplate = argValue;
                                break;
                            default:
                                throw Error(`Unknown Decorator '${argIdent}'`);
                        }
                    });
                    break;
                }
...
export function flowParseBlock(
    debug: Debug,
    v: number,
    flow: Flow,
    s: ts.Node,
    connector: [Connectable, boolean, string]
): void {
    if (!s) return;
    debug?.push();
        ...
        switch (c1.kind) {
            case sk.FirstStatement: {
                ...
                if (
                    ...
                    throw Error('flowParseBlock: not sure NodeFlags/Length');
                ...
                if (
                    func.expression.kind !== sk.PropertyAccessExpression &&
                    (func.expression as ts.PropertyAccessExpression).expression.kind !== sk.ThisKeyword
                )
                ...
                if (funcName === 'query' && arg0text.startsWith('ACTION'))
                    nextConnector = actionCallParse(debug, v, flow, stmt, func);
                else if (funcName === 'query' && arg0text.startsWith('APEX'))
                    nextConnector = apexPluginCallParse(debug, v, flow, stmt, func);
                else if (funcName === 'query' && arg0text.startsWith('INSERT'))
                    nextConnector = recordCreateParse(debug, v, flow, stmt, func);
                else if (funcName === 'query' && arg0text.startsWith('DELETE'))
                    nextConnector = recordDeleteParse(debug, v, flow, stmt, func);
                else if (funcName === 'query' && arg0text.startsWith('SELECT'))
                    nextConnector = recordLookupParse(debug, v, flow, stmt, func);
                else if (funcName === 'query' && arg0text.startsWith('UPDATE'))
                    nextConnector = recordUpdateParse(debug, v, flow, stmt, func);
                ...
                else if (funcName === 'step') nextConnector = stepParse(debug, v, flow, stmt, func);
                else if (funcName === 'wait') nextConnector = waitParse(debug, v, flow, stmt, func);
...
export function flowBuild(debug: Debug, s: Flow): ts.Node {
    ...
    for (const t of [
        ...
        toArray(s[name]).forEach((e) => {
            ...
            switch (k) {
                case 'r':
                    resources.push(e as Resource);
                    break;
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
                case 'e':
                    elements[e.name] = e as Element;
                    break;
                default:
                    break;
            }
    ...
    if (s.runInMode)
        flowArgs.push(
            sf.createBinaryExpression(
                sf.createIdentifier('runInMode'),
                sk.EqualsToken,
                sf.createStringLiteral(s.runInMode, true)
            )
        );
    if (s.status !== 'Active')
        flowArgs.push(
            sf.createBinaryExpression(
                sf.createIdentifier('status'),
                sk.EqualsToken,
                sf.createStringLiteral(s.status, true)
            )
        );
    if (s.environments)
        flowArgs.push(
            sf.createBinaryExpression(
                sf.createIdentifier('environments'),
                sk.EqualsToken,
                sf.createStringLiteral(s.environments, true)
            )
        );
    ...
    if (s.sourceTemplate)
        decorators.push(
            sf.createDecorator(
                sf.createCallExpression(sf.createIdentifier('sourceTemplate'), undefined, [
                    sf.createStringLiteral(s.sourceTemplate, true),
                ])
            )
        );
